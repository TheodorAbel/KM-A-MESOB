...
export function ArticleEditorModal({ 
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'prose prose-slate max-w-none focus:outline-none min-h-[150px] sm:min-h-[200px] p-3 sm:p-4',
      },
    },
  });
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center sm:items-start sm:pt-4 sm:pb-4">
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={onClose} />
      <div className="relative bg-white w-full h-full sm:h-auto sm:max-h-[calc(100vh-2rem)] sm:max-w-4xl sm:rounded-2xl shadow-xl overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-200 bg-slate-50 sm:bg-white">
          <h2 className="text-base sm:text-xl font-bold text-slate-800 truncate pr-2">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs sm:text-sm font-medium text-slate-700 mb-1">Tags (comma separated)</label>
            <input
              type="text"
              value={tags}
              onChange={(e) => setTags(e.target.value)}
              placeholder="e.g., troubleshooting, resolution"
              className="w-full px-3 sm:px-4 py-2 sm:py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
            />
          </div>

          <div>
            <label className="block text-xs sm:text-sm font-medium text-slate-700 mb-2">Content</label>
            <div className="border border-slate-200 rounded-lg overflow-hidden">
              <div className="flex items-center gap-1 p-1.5 sm:p-2 border-b border-slate-200 bg-slate-50 overflow-x-auto">
                <button
                  onClick={() => editor?.chain().focus().toggleBold().run()}
                  className={`p-1.5 sm:p-2 rounded hover:bg-slate-200 ${editor?.isActive('bold') ? 'bg-slate-200' : ''}`}
                >
                  <span className="font-bold text-xs sm:text-sm">B</span>
                </button>
                <button
                  onClick={() => editor?.chain().focus().toggleItalic().run()}
                  className={`p-1.5 sm:p-2 rounded hover:bg-slate-200 ${editor?.isActive('italic') ? 'bg-slate-200' : ''}`}
                >
                  <span className="italic text-xs sm:text-sm">I</span>
                </button>
                <button
                  onClick={() => editor?.chain().focus().toggleHeading({ level: 2 }).run()}
                  className={`p-1.5 sm:p-2 rounded hover:bg-slate-200 ${editor?.isActive('heading', { level: 2 }) ? 'bg-slate-200' : ''}`}
                >
                  <span className="font-bold text-xs sm:text-sm">H2</span>
                </button>
                <button
                  onClick={() => editor?.chain().focus().toggleBulletList().run()}
                  className={`p-1.5 sm:p-2 rounded hover:bg-slate-200 ${editor?.isActive('bulletList') ? 'bg-slate-200' : ''}`}
                >
        ...
        <div className="flex items-center justify-end gap-2 sm:gap-3 px-4 sm:px-6 py-3 sm:py-4 border-t border-slate-200 bg-slate-50">
          <button onClick={onClose} className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg font-medium text-sm">
            Cancel
          </button>
          <button
