...
export const mockIssueTickets: IssueTicket[] = [
  {
    id: '1',
    title: 'Telebirr webhook signature validation failing in production',
    description: 'Telebirr payment webhook signatures failing HMAC validation after their API update. Payments completing but confirmations not processing. Affecting all government fee collections.',
    status: 'Open',
    priority: 'Critical',
    category: 'FinTech',
    reportedById: '1',
    ...
    rootCause: 'Telebirr updated their HMAC-SHA256 signature algorithm without notice',
    preventativeMeasures: '',
    postMortemGenerated: false,
  },
  {
    id: '2',
    title: 'E-Tax portal: Database indexing failure causing 500 errors',
    description: 'PostgreSQL queries on taxpayer lookup table timing out during peak hours. Error logs show sequential scans on 10M+ row table. Affecting all business registration workflows.',
    status: 'In Progress',
    priority: 'High',
    category: 'Database',
    reportedById: '2',
    ...
    rootCause: 'Missing index on taxpayer.tin column after recent schema migration',
    preventativeMeasures: 'Add automated index health checks to CI/CD pipeline',
    postMortemGenerated: false,
  },
  {
    id: '3',
    title: 'Kebele ID API: Race condition in national ID caching layer',
    description: 'Intermittent authentication failures when multiple citizen requests hit cache simultaneously. Appears to corrupt Redis cache entries for national ID lookups.',
    status: 'Resolved',
    priority: 'High',
    category: 'Integration',
    reportedById: '4',
    assignedToId: '2',
    resolutionNotes: 'Race condition in cache update logic. Implemented distributed locking with Redlock algorithm.',
    createdAt: '2025-04-15T09:00:00Z',
    updatedAt: '2025-04-18T15:00:00Z',
    rootCause: 'Cache invalidation race condition when concurrent requests update same key',
    preventativeMeasures: 'Added Redis distributed locking; implemented cache versioning',
    postMortemGenerated: true,
  },
  {
    id: '4',
    title: 'Ethio Telecom SMS: USSD callback timeout handling broken',
    description: 'USSD session timeouts not being handled correctly. Citizens completing payment flow but sessions expiring before confirmation. Callback URLs returning 408 errors.',
    status: 'In Progress',
    priority: 'High',
    category: 'Integration',
    reportedById: '1',
    ...
    updatedAt: '2025-04-21T16:45:00Z',
    rootCause: 'USSD gateway timeout set to 30s but our session handler expires at 60s',
    preventativeMeasures: '',
    postMortemGenerated: false,
  },
  {
    id: '5',
    title: 'CBE Payment: Idempotency key collision causing duplicate charges',
    description: 'Two identical payment requests processed within milliseconds causing double charges for 3 citizens. Idempotency key generation using timestamp alone is insufficient.',
    status: 'Open',
    priority: 'Critical',
    category: 'FinTech',
    reportedById: '6',
    ...
    rootCause: 'Under investigation - likely idempotency key collision under high concurrency',
    preventativeMeasures: '',
    postMortemGenerated: false,
  },
  {
    id: '6',
    title: 'E-Passport API: SOAP/XML parsing memory leak in production',
    description: 'Memory usage increasing 15% daily on e-Passport integration service. Heap dumps show XML parser retaining DOM nodes. Service requires daily restart.',
    status: 'Open',
    priority: 'Medium',
    category: 'API',
    reportedById: '4',
    ...
    rootCause: 'XML parser not releasing DOM nodes after processing responses',
    preventativeMeasures: '',
    postMortemGenerated: false,
  },
];
